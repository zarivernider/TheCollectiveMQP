...
#include "ci_kheperaiv_turret_actuator.h"
...
namespace argos {
  ...
  const CRange<SInt32> CCI_KheperaIVTurretActuator::SPEED_RANGE(-4,4);
  const CRange<Real>   CCI_KheperaIVTurretActuator::NORMALIZED_SPEED_RANGE(-1.0,1.0);
  ...
  int LuaTurretSetRotation(lua_State* pt_lua_state) {
    ...
    /* Perform action */
    CLuaUtility::GetDeviceInstance<CCI_KheperaIVTurretActuator>(pt_lua_state, "turret")->
  ...
  void CCI_KheperaIVTurretActuator::SetActiveWithRotation(const CRadians& c_angle) {
    SetPositionControlMode();
    SetRotation(c_angle);
  }
  ...
  void CCI_KheperaIVTurretActuator::SetSpeedControlMode() {
  ...
  void CCI_KheperaIVTurretActuator::SetPositionControlMode() {
  ...
  void CCI_KheperaIVTurretActuator::SetPassiveMode() {
...
#ifdef ARGOS_WITH_LUA
  void CCI_KheperaIVTurretActuator::CreateLuaState(lua_State* pt_lua_state) {
