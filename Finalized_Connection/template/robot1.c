...
int main( int arc, char *argv[])
{
  ...
  if(!initGripper())
  {
    ...
    // sleep(1);
    // destroy();
    // cgripper_Turret_Set_Proportional(5325);
    cgripper_Turret_Force_Tolerance(25000);
    cgripper_Turret_Push();
    // while(1){
    //   unsigned short parallel = cgripper_ForceSensor_Get_Parallel_Force();
    //   unsigned short perpendicular = cgripper_ForceSensor_Get_Perpendicular_Force();
    //   printf("Parallel Force Sensor reading : %hu\n", parallel);
    //   printf("Perpendicular Force Sensor reading : %hu\n", perpendicular);
    // }
	}
	else{
